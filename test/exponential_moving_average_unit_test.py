...
def calculate_ema_adjusted_by_panads(stock_price_holder = StockPriceHolder, window_size = int, alpha = float):
    close_list = stock_price_holder.get_close_list()
    df = pd.DataFrame({'val': close_list})
    ema_list = df.ewm(alpha=alpha, adjust=True).mean()
    ema_list = list(ema_list['val'])
...
class TestEMA(unittest.TestCase):
    ...
    def test_win_size_less_than_2_sts_method(self):
        stock_name = "MicroSoft"
        stock_no = "MSFT"
        window_size = 1
        file_name = "C:/Users/insan/Documents/Project/Python/Project/quant/test/test_data/HistoricalData_MSFT.csv"
        ...
        with self.assertRaises(Exception) as context:
            ema = ExponentialMovingAverage.calculate_moving_average(stock_price_holder.get_close_list(), window_size)
    ...
    def test_aapl_adjusted_aapl_5(self):
        stock_name = "Apple"
        stock_no = "AAPL"
        window_size = 5
        file_name = "C:/Users/insan/Documents/Project/Python/Project/quant/test/test_data/HistoricalData_AAPL.csv"
        ...
        ema = ExponentialMovingAverage(stock_price_holder, window_size, alpha, True)
        ema_wait_test = ema.get_moving_average_list()
        ema_test_list = []
        for num in ema_wait_test:
            ema_test_list.append(float("{:.4f}".format(num)))

        ema_list_right = calculate_ema_adjusted_by_panads(stock_price_holder, window_size, alpha)
        period_list = stock_price_holder.get_date_list()
    ...
    def test_aapl_adjusted_aapl_14(self):
        stock_name = "Apple"
        stock_no = "AAPL"
        window_size = 14
        file_name = "C:/Users/insan/Documents/Project/Python/Project/quant/test/test_data/HistoricalData_AAPL.csv"
    ...
    def test_aapl_adjusted_win_14(self):
        stock_name = "MicroSoft"
        stock_no = "MSFT"
        window_size = 14
        file_name = "C:/Users/insan/Documents/Project/Python/Project/quant/test/test_data/HistoricalData_MSFT.csv"
